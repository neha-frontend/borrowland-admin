...
import { loanManagementColumn } from 'constants/tableColumn';

import '../viewcommon.css';
import { platformManageLoanList } from 'store/actions';
...
const LoanManagement = () => {
  ...
  const { isLoading, platformManagementLoanList } = useSelector(state => state.platformManagement);
  ...
  const submit = (e, values) => {
    ...
    dispatch(platformManageLoanList({ query: `page=${currentPage}&limit=${count}`, data: temp }));
  };

  useEffect(() => {
    if (startDate) {
      dispatch(
        platformManageLoanList({
  ...
  const onPageChange = page => {
    setCurrentPage(page);
    dispatch(platformManageLoanList({ query: `page=${page}&limit=${count}`, data: filteredData }));
  };
  ...
  useEffect(() => {
    if (platformManagementLoanList?.list) {
      platformManagementLoanList?.list?.forEach(ele => {
        const obj = ele;
        obj.createdAt = ele?.createdAt ? new Date(ele.createdAt).toLocaleString() : '-';
        obj.LoanAmount = ele?.loan?.amount || '-';
        obj.token = (ele?.loan?.token).toUpperCase() || '-';
        obj.userName = ele?.userdata?.fullName || '-';
      });
      const paginationConfigTemp = {
        currentPage,
        pageCount: platformManagementLoanList?.totalPage,
        count,
        itemCount: platformManagementLoanList?.totalItem,
        onPageChange,
  ...
  useEffect(() => {
    dispatch(platformManageLoanList({ query: `page=${currentPage}&limit=${count}` }));
  }, []);
  ...
  return (
    <div className="page-content">
      ...
      <Row>
        <UserWidget reports={topCardData?.loanManagement} loading={isCardLoading} />
      </Row>
      <Row>
          ...
          <Row className="row mb-4">
            ...
            <Col sm={12} lg={3} md={4}>
              ...
              <AvField
                ...
                // onChange={e => (adminObj.name = e?.target?.value)}
                placeholder="Type of Loan Assets"
              >
      ...
      /> */}
            ...
            <DatatableTables
              column={loanManagementColumn}
              //   handleClick={handleClick}
              //   isSend
              //   isEdit
              //   isView
              //   isBlock
              row={isLoading ? 'loading' : platformManagementLoanList?.list}
              hidePaging
