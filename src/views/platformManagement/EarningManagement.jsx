...
import { earningManagementColumn } from 'constants/tableColumn';

import '../viewcommon.css';
import { useDispatch, useSelector } from 'react-redux';
import { platformManageEarnList } from 'store/actions';
...
const EarningManagement = () => {
  ...
  const { isLoading, platformManagementEarnList } = useSelector(state => state.platformManagement);
  ...
  const submit = (e, values) => {
    ...
    dispatch(platformManageEarnList({ query: `page=${currentPage}&limit=${count}`, data: temp }));
  };
  useEffect(() => {
    if (startDate) {
      dispatch(
        platformManageEarnList({
  ...
  const onPageChange = page => {
    setCurrentPage(page);
    dispatch(platformManageEarnList({ query: `page=${page}&limit=${count}`, data: filteredData }));
  };
  ...
  useEffect(() => {
    if (platformManagementEarnList?.list) {
      platformManagementEarnList?.list?.forEach(ele => {
        const obj = ele;
        obj.userFullname = ele?.userdata?.fullName;
        // obj.profit = ele?.profit || '-';
        // eslint-disable-next-line no-underscore-dangle
        obj.txnType = ele?.token._type === '_Swap' ? 'Swap' : ele?.token._type || '-';
      ...
      const paginationConfigTemp = {
        currentPage,
        pageCount: platformManagementEarnList?.totalPage,
        count,
        itemCount: platformManagementEarnList?.totalItem,
        onPageChange,
  ...
  useEffect(() => {
    dispatch(platformManageEarnList({ query: `page=${currentPage}&limit=${count}` }));
  }, []);
    ...
    <div className="page-content">
      <div className="d-flex align-items-baseline justify-content-between">
        <div className="d-flex">
          <div>
            <Breadcrumb name="Earning Management" />
          </div>
        </div>
      </div>
      <RenderIf>
      ...
      <Row>
        <UserWidget reports={topCardData?.earningManagement} loading={isCardLoading} />
      </Row>
      <Row>
            ...
            <Col sm={12} lg={3} md={4}>
              ...
              <AvField
                ...
                validate={{
                  pattern: {
                    value: /^[0-9]+$/,
                    errorMessage: 'Earning must be a number',
                  },
                }}
              />
            </Col>
      ...
      /> */}
            ...
            <DatatableTables
              column={earningManagementColumn}
              //   handleClick={handleClick}
              //   isSend
              //   isEdit
              //   isView
              //   isBlock
              row={isLoading ? 'loading' : platformManagementEarnList?.list}
              hidePaging
